...
type TalkChannelHandlerEvents<T extends Channel> = ChannelEvents<
  T,
  ChannelUserInfo
>;
 ...
 */
export class TalkChannelHandler<T extends Channel>
  implements Managed<TalkChannelHandlerEvents<T>>
{
  constructor(
    private _channel: T,
  ...
  private _callEvent<E extends keyof TalkChannelHandlerEvents<T>>(
    ...
    event: E,
    ...args: Parameters<TalkChannelHandlerEvents<T>[E]>
  ) {
  ...
  private _msgHandler(
    ...
    if (msgData.authorNickname) {
      ...
      if (userInfo && userInfo.nickname !== msgData.authorNickname) {
        this._store.updateUserInfo(chatLog.sender, {
          nickname: msgData.authorNickname,
        });
      }
    }
  ...
  private _chatReadHandler(
    readData: DefaultRes & DecunreadRes,
    parentCtx: EventContext<TalkChannelHandlerEvents<T>>,
  ) {
  ...
  private _metaChangeHandler(
    metaData: DefaultRes & ChgMetaRes,
    parentCtx: EventContext<TalkChannelHandlerEvents<T>>,
  ) {
  ...
  private _msgDeleteHandler(
    ...
    this._chatListStore
      ...
      .then(async () => {
        ...
        await this._chatListStore.updateChat(delAllFeed.logId, {
          type: chat.type | DELETED_MESSAGE_OFFSET,
        });
      })
      .then();
  }
  ...
  async pushReceived(
    ...
    switch (method) {
      ...
      case 'FEED':
        ...
        break;
      case 'DECUNREAD':
        await this._chatReadHandler(
          data as DefaultRes & DecunreadRes,
          parentCtx,
        );
        break;
      case 'CHGMETA':
        await this._metaChangeHandler(
          data as DefaultRes & ChgMetaRes,
          parentCtx,
        );
        break;
      case 'DELMEM':
...
export class TalkChannelListHandler<T extends Channel>
  implements Managed<ChannelListEvent<T>>
{
  constructor(
  ...
  private _callEvent<E extends keyof ChannelListEvent<T>>(
    ...
    event: E,
    ...args: Parameters<ChannelListEvent<T>[E]>
  ) {
    this._emitter.emit(event, ...args);
    parentCtx.emit(event, ...args);
  }
